...
final class DemoListWorkflowTests: XCTestCase {
	...
	func testUpdateDemos() {
		...
		).send(action: .updateDemos).verifyState { state in
			XCTAssert(state.updateWorker.isWorking)
		}.assertNoOutput()
	}
	...
	func testRenderingSelectDemo() throws {
		...
		try DemoList.Workflow(
			...
			action: DemoList.Workflow.Action.demo(demo)
		).assert(
			output: demo
		)
	}
	...
	func testRenderingAlertLoadError() throws {
		...
		try DemoList.Workflow(service: api).renderTester(
			...
			XCTAssert(state.updateWorker.isReady)
	...
	func testUpdateWorkerFailureLoadError() {
	...
	func testUpdateWorkerFailureSleepError() {
